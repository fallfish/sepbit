...
public class SepBIT extends Separator {
  // Since the simulator never stores a real block, we maintain a full mapping here to speedup simulation.
  // To store the accessIDs in real disk (thus no memory overhead), two methods can be adopted:
  // 1. leveraging the spare space of disk to store the access IDs (an example design is like FADaC.systor19)
  // 2. store all accessIDs inside a segment metadata (e.g., infoFile, appendInfo() in segment.SegmentMeta class.)
  //
  // Note that this indexmap is memory efficient (memory cache + sparse on-disk array) by default to speed up simulation
  public IndexMap lastAccess;

  // Maintain an on-disk FIFO and an in-memory mapping to illustrate the memory overhead of SepBIT with simulation
  ...
  public int classify(boolean isGcAppend, long lba) {
    ...
    if (!isGcAppend) {
      ...
      if (currentCollectedTemp == 0) {
        ...
        //double exp = Math.pow(16, 1.0 / numPoints);
        double exp = 4;
